...
import editEvents from "./api/editEvents";

const EventEditFinnish = props => {
  ...
  useEffect(() => {
    ...
    const { finishData } = JSON.parse(localStorage.getItem(eventID));
    setLocation(finishData.location);
  }, []);
  ...
  const handleFinnish = () => {
    ...
    if (!finnishError) {
      ...
      editEvents(eventID, props.eventData);
  ...
  return (
    <div className="event-create__main">
      ...
      <div className="creation-wizard">
        ...
        <Link to={{
            pathname: `/event/edit/contacts/${props.match.params.id}`,
            prevProps: "previous"
          }}>
